...
class AlignNet(nn.Module):
    def __init__(self):
        ...
        self.final = nn.Sequential(
                ConvLayer(in_channels=3,out_channels=3,kernel_size=9),
                ConvLayer(in_channels=3,out_channels=3,kernel_size=3),
                ConvLayer(in_channels=3,out_channels=3,kernel_size=1)
                )

        self.R_tunnel = nn.Sequential(
                ResUnit(3,1),
                ResUnit(3,3),
                ResUnit(3,1)
                )

        self.T_tunnel = nn.Sequential(
                ResUnit(3,7),
                ResUnit(3,3),
                ResUnit(3,1)
                )
    ...
    def forward(self,R,T):
        ...
        T_masked = T_masked + T_feat
        
        T_rslt = self.up4(self.up3(self.up2(self.up1(T_masked))))
